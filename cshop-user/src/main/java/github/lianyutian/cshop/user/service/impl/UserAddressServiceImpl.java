...
public class UserAddressServiceImpl implements UserAddressService {
    ...
    public boolean updateUserAddress(AddressEditParam addressEditParam) {
        ...
        int rows = userAddressMapper.update(userAddress,
    ...
    private void setDefaultStatusIfRequired(UserAddress userAddress) {
        if (Objects.equals(userAddress.getDefaultStatus(), AddressStatusEnum.DEFAULT_STATUS.getCode())) {
            // 设置默认的为非默认状态
    ...
    private String formatUserAddressForLog(UserAddress userAddress) {
        // 只打印必要的字段，避免泄露敏感信息
        return "id=" + userAddress.getId() + ", userId=" + userAddress.getUserId() + ", defaultStatus=" + userAddress.getDefaultStatus();
    }
}
