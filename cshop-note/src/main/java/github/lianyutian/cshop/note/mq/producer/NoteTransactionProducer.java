package github.lianyutian.cshop.note.mq.producer;
 ...
 *
 * @author lianyutian
 * @since 2025-01-14 14:52:03
 * @version 1.0
 */
@Getter
@Component
@Slf4j
public class NoteTransactionProducer {

  private final TransactionMQProducer producer;

  @Autowired
  public NoteTransactionProducer(
      RocketMQConfig rocketMQConfig, RPCHook rpcHook, NoteAddListener noteAddListener) {
    // 初始化事务生产者客户端，设置对应的生产者组
    producer = new TransactionMQProducer(NoteRocketMQConstant.NOTE_UPDATE_PRODUCER_GROUP, rpcHook);
    // 设置 nameserver
    producer.setNamesrvAddr(rocketMQConfig.getNameServer());
    producer.setTransactionListener(noteAddListener);
    // 启动生产者服务
    start();
  }
  ...
  public void sendTransactionMessage(
      String topic, String tag, String message, Object arg, NoteMessageType type) {
   ...
   * @param type 类型
   */
  public void sendMessage(String topic, String tag, String message, NoteMessageType type) {
  ...
  public void sendDelayMessage(
      String topic, String tag, String message, DelayLevel delayTimeLevel, NoteMessageType type) {
    Message msg = new Message(topic, tag, message.getBytes(StandardCharsets.UTF_8));
    try {
